...
class Creator extends Barcode\Creator
{
    ...
    public function __construct($data, array $properties = [])
    {
        ...
        if (!preg_match('/^[- *$%.\/+a-z0-9]+$/i', $data)) {
            throw new Throwable\InvalidArgument\Exception('Invalid character in data string. Expected an data in the Code 39 subset, but got ":data" value.', [':data' => $data]);
        }
    ...
    public function render()
    {
        if ($this->image === null) {
            ...
            $width = $length * 16;
            $height = 33;
            $padding = [ // Follows CSS padding rules: top right bottom left
                0, // top
                0, // right
    ...
    public static function decode(IO\File $file)
    {
        ...
        switch ($ext) {
            ...
            case 'jpg':
                ...
                break;
            default:
                throw new Throwable\InvalidArgument\Exception('Unrecognized file format. File name must have either a png or jpg extension.', ['file' => $file]);
        }
