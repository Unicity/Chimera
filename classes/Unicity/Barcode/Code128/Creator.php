...
class Creator extends Barcode\Creator
{
    ...
    public function __construct($data, $charset = 'C')
    {
        ...
        if (!preg_match('/^[abc]$/i', $charset)) {
            throw new Throwable\InvalidArgument\Exception('Invalid character set declared. Expected either an "A", "B", or "C", but got ":charset".', [':charset' => $charset]);
        }
            ...
            switch ($this->charset) {
                ...
                case 'B':
                    for ($index = 0; $index < $length; $index++) {
                        ...
                        if (!isset(self::$values[$this->charset][$char])) {
                            throw new Throwable\InvalidArgument\Exception('Invalid character in input string. Character ":char" cannot be encoded using character set ":set".', [':set' => $this->charset, ':char' => $char]);
                        }
                ...
                case 'C':
                    for ($index = 0; $index < $length; $index += 2) {
                        ...
                        if (!isset(self::$values[$this->charset][$char])) {
                            throw new Throwable\InvalidArgument\Exception('Invalid character in input string. Reason: Character ":char" cannot be encoded using character set ":set".', [':set' => $this->charset, ':char' => $char]);
                        }
                        $value = self::$values[$this->charset][$char];
                        $code .= self::$patterns[$value];
                        $checksum += ($index * $value);
