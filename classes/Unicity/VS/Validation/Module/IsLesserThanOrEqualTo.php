...
namespace Unicity\VS\Validation\Module {
	...
	class IsLesserThanOrEqualTo extends VS\Validation\Module {
		...
		public function process(BT\Entity $entity, array $paths): VS\Validation\Feedback {
			...
			foreach ($paths as $path) {
				...
				if ($v1 > $v2) {
					$feedback->addViolation(VS\Validation\RuleType::MALFORMED, [$path], 'Field must be lesser than or equal to ":value".', [':value' => $v2]);
				}
			}
