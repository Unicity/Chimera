...
class File extends IO\Resource
{
    ...
    public function __construct($uri)
    {
        ...
        if (!Common\StringRef::isTypeOf($uri)) {
            throw new Throwable\InvalidArgument\Exception('Unable to handle argument. Argument must be a string, but got :type.', [':type' => gettype($uri)]);
        }
    ...
    public function exists(): bool
    {
        ...
        } elseif (in_array($this->uri, [self::INPUT, self::STDERR, self::STDIN, self::STDOUT]) || preg_match('/^data\\:\\/\\//', $this->uri)) {
            $exists = false;
    ...
    {
        if (in_array($this->uri, [self::INPUT, self::STDERR, self::STDIN, self::STDOUT]) || preg_match('/^data\\:\\/\\//', $this->uri)) {
    ...
    {
        if ($this->ext === null) {
            $this->ext = (in_array($this->uri, [self::INPUT, self::STDIN]) || preg_match('/^data\\:\\/\\//', $this->uri))
        ...
        if ($handle) {
            ...
            if (preg_match('/^<\?xml\s+[^?>]+\?>/', $buffer)) {
                if (preg_match('/<plist/', $buffer)) {
                    $ext = 'plist';
                } elseif (preg_match('/<objects/', $buffer)) {
                    $ext = 'spring';
                } elseif (preg_match('/<soap:Envelope/', $buffer)) {
                    $ext = 'soap';
                } elseif (preg_match('/<wddxPacket/', $buffer)) {
                    $ext = 'wddx';
                } else {
                    $ext = 'xml';
                }
            } elseif (preg_match('/^<html/', $buffer)) {
                $ext = 'html';
            } elseif (preg_match('/^<\?php/', $buffer)) {
                $ext = 'php';
            } elseif (preg_match('/^[{]/', $buffer)) {
                $ext = 'json';
            } elseif (preg_match('/^[^=]+=.+/', $buffer) || preg_match('/^#.*$/', $buffer)) {
                $ext = 'properties';
            } elseif (preg_match('/^(.*,)+/', $buffer) || preg_match('/^(.*\|)+/', $buffer)) {
                $ext = 'csv';
            }
