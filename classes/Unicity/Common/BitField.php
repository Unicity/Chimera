...
class BitField extends Core\AbstractObject implements \Countable
{
    ...
    public function __get($field)
    {
        if (!array_key_exists($field, $this->values)) {
            throw new Throwable\InvalidProperty\Exception('Unable to get the specified property. Property :field is either inaccessible or undefined.', [':field' => $field]);
        }
    ...
    protected function map($value)
    {
        ...
        if ($start > $this->boundary) {
            throw new Throwable\Runtime\Exception('Invalid bit-field pattern. Pattern exceeds the bit boundary of :boundary.', [':pattern' => $this->pattern, ':boundary' => $this->boundary]);
        }
    }
    ...
    protected static function unpack($value, $boundary)
    {
        ...
        } elseif (is_object($value) && ($value instanceof Common\BitField)) {
