...
class ArrayList extends Common\ArrayList implements Common\Mutable\IList
{
    ...
    public function insertValue(int $index, $value): bool
    {
        $count = $this->count();
        if ($index > $count) {
            throw new Throwable\OutOfBounds\Exception('Unable to insert value. Invalid index specified', [':index' => $index, ':value' => $value]);
        } elseif ($index == $count) {
    ...
    public function removeIndex($index)
    {
        ...
        if (!array_key_exists($index, $this->elements)) {
            throw new Throwable\OutOfBounds\Exception('Unable to remove element. Invalid index specified', [':index' => $index]);
        }
    ...
    public function removeIndexes($indexes)
    {
        ...
        foreach ($indexes as $index) {
            if (!is_integer($index)) {
                throw new Throwable\InvalidArgument\Exception('Unable to remove element. Index must be an integer.', [':type' => gettype($index)]);
            } elseif (!array_key_exists($index, $elements)) {
                throw new Throwable\OutOfBounds\Exception('Unable to remove element. Invalid index specified', [':index' => $index]);
            } else {
    ...
    public function removeRangeOfIndexes($sIndex, $eIndex)
    {
        ...
        if (is_integer($sIndex) && is_integer($eIndex)) {
            ...
            throw new Throwable\InvalidRange\Exception('Unable to remove range. Invalid range start from :start and ends at :end', [':start' => $sIndex, ':end' => $eIndex]);
        }

        throw new Throwable\InvalidArgument\Exception('Unable to remove range. Either :start or :end is of the wrong data type.', [':start' => gettype($sIndex), ':end' => gettype($eIndex)]);
    }
    ...
    public function retainIndex($index)
    {
        ...
        throw new Throwable\OutOfBounds\Exception('Unable to retain index. Invalid index specified', [':index' => $index]);
    }
