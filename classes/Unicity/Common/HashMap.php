...
class HashMap extends Core\AbstractObject implements Common\IMap
{
    ...
    protected function getKey($key)
    {
        if (is_object($key) && ($key instanceof Common\StringRef)) {
            ...
            throw new Throwable\InvalidArgument\Exception('Invalid key specified. Expected integer or string, but got ":type" instead.', [':type' => Core\DataType::info($key)->type]);
        }
    ...
    public function getValue($key)
    {
        ...
        if (!array_key_exists($key, $this->elements)) {
            throw new Throwable\KeyNotFound\Exception('Unable to get element. Key ":key" does not exist.', [':key' => $key]);
        }
    ...
    {
        throw new Throwable\UnimplementedMethod\Exception('Invalid call to member function. Result set cannot be modified.', [':offset' => $this->getKey($offset), ':value' => $value]);
    }
    ...
    {
        throw new Throwable\UnimplementedMethod\Exception('Invalid call to member function. Result set cannot be modified.', [':offset' => $this->getKey($offset)]);
    }
