...
class Parser extends Core\AbstractObject
{
    ...
    protected function SyntaxError(): void
    {
        ...
        if (is_null($tuple)) {
            ...
            $this->WriteError('VLD Parse error: syntax error, unexpected token \':token\' of type \':type\' encountered at :index.', [':index' => $tuple->index, ':token' => (string) $tuple->token, ':type' => (string) $tuple->type]);
        }
    }
