		...
		public function process(BT\Entity $entity, array $paths): VLD\Parser\Feedback {
			...
			foreach ($paths as $path) {
				...
				if (is_string($v1) && (strlen($v1) >= $v2)) {
					$feedback->addViolation(VLD\RuleType::mismatch(), VLD\Code::VALUE_IS_LT_LENGTH, [$path], ['{{length}}' => $v2]);
				}
				else if (($v1 instanceof Common\IList) && ($v1->count() >= $v2)) {
					$feedback->addViolation(VLD\RuleType::mismatch(), VLD\Code::VALUE_IS_LT_SIZE, [$path], ['{{size}}' => $v2]);
				}
			}
