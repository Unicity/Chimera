...
class IsGreaterThanLength extends VLD\Module
{
    public function process(BT\Entity $entity, array $paths): VLD\Parser\Feedback
    {
        ...
        foreach ($paths as $path) {
            ...
            if (is_string($v1) && (strlen($v1) <= $v2)) {
                $feedback->addViolation(VLD\RuleType::mismatch(), VLD\Code::VALUE_IS_GT_LENGTH, [$path], ['{{ length }}' => $v2]);
            } elseif (($v1 instanceof Common\IList) && ($v1->count() <= $v2)) {
                $feedback->addViolation(VLD\RuleType::mismatch(), VLD\Code::VALUE_IS_GT_SIZE, [$path], ['{{ size }}' => $v2]);
            }
        }
