...
/**
 * This class represents the rule definition for a "boolean variable" token, which the
 ...
 * @package VLD
 */
class BooleanVariable extends Core\AbstractObject implements Lexer\Scanner\ITokenRule
{
    /**
    ...
    public function process(IO\Reader $reader): ?Lexer\Scanner\Tuple
    {
        ...
        if (($char !== null) && ($char === '?')) {
            ...
            if (($next !== null) && preg_match('/^[_a-z0-9]$/i', $next)) {
                ...
                $tuple = new Lexer\Scanner\Tuple(VLD\Scanner\TokenType::variable_boolean(), new Common\StringRef($token), $index);
