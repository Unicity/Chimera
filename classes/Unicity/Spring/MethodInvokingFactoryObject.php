    ...
    public function afterPropertiesSet()
    {
        if ($this->targetClass !== null) {
            if (!method_exists($this->targetClass, $this->targetMethod)) {
                throw new Throwable\Parse\Exception('Unable to process Spring XML. Expected a valid callable, but got ":class" and ":method".', [':class' => $this->targetClass, ':method' => $this->targetMethod]);
            }
            $this->callable = [$this->targetClass, $this->targetMethod];
        } elseif ($this->targetClass !== null) {
            if (!method_exists($this->targetObject, $this->targetMethod)) {
                throw new Throwable\Parse\Exception('Unable to process Spring XML. Expected a valid callable, but got ":class" and ":method".', [':class' => Core\DataType::info($this->targetObject)->type, ':method' => $this->targetMethod]);
            }
            $this->callable = [$this->targetObject, $this->targetMethod];
        } else {
            if (in_array($this->targetMethod, ['eval'])) {
                throw new Throwable\Parse\Exception('Unable to process Spring XML. Expected a valid callable, but got ":method".', [':method' => $this->targetMethod]);
            }
