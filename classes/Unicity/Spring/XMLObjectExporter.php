...
class XMLObjectExporter extends Core\AbstractObject
{
    ...
    public function __construct($objects)
    {
        if (is_array($objects)) {
            if (static::isDictionary($objects)) {
                $objects = [new Common\HashMap($objects)];
            }
            $this->objects = $objects;
        } elseif (is_object($objects) && ($objects instanceof Common\IList)) {
        ...
        $this->encoding = [Core\Data\Charset::UTF_8_ENCODING, Core\Data\Charset::UTF_8_ENCODING];
        $this->index = 0;
        $this->lookup = null;
    ...
    {
        if (!in_array($name, ['encoding', 'file_name', 'objects', 'prototype'])) {
        ...
        foreach ($data as $value) {
            ...
            switch ($type) {
                ...
                case 'object':
                    ...
                    } elseif ($value instanceof Common\IMap) {
                        $this->addMap($root, $child, $value);
                    } elseif ($value instanceof Common\ISet) {
    ...
    protected function addEntry($root, $node, $data, $key)
    {
        ...
        switch ($type) {
            ...
            case 'object':
                ...
                } elseif ($data instanceof Common\IMap) {
                    $this->addMap($root, $child, $data);
                } elseif ($data instanceof Common\ISet) {
                    $this->addSet($root, $child, $data);
                } elseif ($data instanceof Common\StringRef) {
                    $this->addValue($root, $child, $data, 'string');
                } elseif ($data instanceof Core\Data\Undefined) {
