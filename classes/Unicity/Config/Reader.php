...
abstract class Reader extends Core\AbstractObject
{
     ...
     */
    abstract public function read($path = null);
    ...
    public static function factory($format, IO\File $file)
    {
        ...
        if (!class_exists($reader)) {
            ...
            foreach ($directories as $dirname) {
                if (!in_array($dirname, ['.', '..']) && (is_dir($directory . DIRECTORY_SEPARATOR . $dirname) && (strtolower($dirname) == $name))) {
            ...
            throw new Throwable\Instantiation\Exception('Unable to initialize class. Unknown reader format, got ":format".', [':format' => $format]);
        }
     ...
     */
    public static function load(IO\File $file, array $metadata = [])
    {
