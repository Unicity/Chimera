...
class Reader extends Config\Reader
{
    ...
    protected function find(\SimpleXMLElement $root, $id)
    {
        if (!Spring\Data\XML\Syntax::isId($id)) {
            throw new Throwable\InvalidArgument\Exception('Invalid argument detected (id: :id).', [':id' => $id]);
        }
    ...
    protected function parseEntryElement(\SimpleXMLElement $root, \SimpleXMLElement $node)
    {
        ...
        if (!isset($attributes['key'])) {
            throw new Throwable\Parse\Exception('Unable to process Spring XML. Tag ":tag" is missing ":attribute" attribute', [':tag' => 'entry', ':attribute' => 'key']);
        }
        ...
        if (count($children) > 0) {
            foreach ($children as $child) {
                ...
                switch ($name) {
                    ...
                    case 'value':
                        ...
                        break;
                    default:
                        throw new Throwable\Parse\Exception('Unable to process Spring XML. Tag ":tag" has invalid child node ":child".', [':tag' => 'entry', ':child' => $name]);
            ...
            if (empty($nodes)) {
                throw new Throwable\Parse\Exception('Unable to process Spring XML. Expected a valid "value-ref" token, but got ":token".', [':token' => $idref]);
            }
    ...
    protected function parseIdRefElement(\SimpleXMLElement $root, \SimpleXMLElement $node)
    {
        ...
        if (isset($attributes['local'])) {
            ...
            if (!Spring\Data\XML\Syntax::isIdref($idref, $root)) {
                throw new Throwable\Parse\Exception('Unable to process Spring XML. Expected a valid "idref" token, but got ":token".', [':token' => $idref]);
            }
            ...
            throw new Throwable\Parse\Exception('Unable to process Spring XML. Tag ":tag" is missing ":attribute" attribute', [':tag' => 'idref', ':attribute' => 'object']);
        }
    }
        ...
        foreach ($children as $child) {
            ...
            switch ($name) {
                ...
                case 'value':
                    ...
                    break;
                default:
                    throw new Throwable\Parse\Exception('Unable to process Spring XML. Tag ":tag" has invalid child node ":child".', [':tag' => 'list', ':child' => $name]);
        ...
        foreach ($children as $child) {
            ...
            switch ($name) {
                case 'entry':
                    ...
                    break;
                default:
                    throw new Throwable\Parse\Exception('Unable to process Spring XML. Tag ":tag" has invalid child node ":child".', [':tag' => 'map', ':child' => $name]);
    ...
    protected function parsePropertyElement(\SimpleXMLElement $root, \SimpleXMLElement $node)
    {
        ...
        if (count($children) > 0) {
            foreach ($children as $child) {
                ...
                switch ($child_name) {
                    ...
                    case 'value':
                        ...
                        break;
                    default:
                        throw new Throwable\Parse\Exception('Unable to process Spring XML. Tag ":tag" has invalid child node ":child".', [':tag' => 'property', ':child' => $child_name]);
            ...
            $nodes = $this->find($root, $idref);
            if (empty($nodes)) {
                throw new Throwable\Parse\Exception('Unable to process Spring XML. Expected a valid "ref" token, but got ":token".', [':token' => $idref]);
            }
    ...
    protected function parseRefElement(\SimpleXMLElement $root, \SimpleXMLElement $node)
    {
        ...
        if (isset($attributes['local'])) {
            ...
            $nodes = $this->find($root, $idref);
            if (empty($nodes)) {
                throw new Throwable\Parse\Exception('Unable to process Spring XML. Expected a valid "local" token, but got ":token".', [':token' => $idref]);
            }
            ...
            $nodes = $this->find($root, $idref);
            if (empty($nodes)) {
                throw new Throwable\Parse\Exception('Unable to process Spring XML. Expected a valid "object" token, but got ":token".', [':token' => $idref]);
            }
            $object = $this->parseOuterObjectElement($root, $nodes[0]);

            return $object;
        } else {
            throw new Throwable\Parse\Exception('Unable to process Spring XML. Tag ":tag" is missing ":attribute" attribute', [':tag' => 'ref', ':attribute' => 'object']);
        }
    }
        ...
        foreach ($children as $child) {
            ...
            switch ($name) {
                ...
                case 'value':
                    ...
                    break;
                default:
                    throw new Throwable\Parse\Exception('Unable to process Spring XML. Tag ":tag" has invalid child node ":child".', [':tag' => 'set', ':child' => $name]);
            ...
            foreach ($children as $child) {
                ...
                switch ($name) {
                    case 'null':
                        ...
                        break;
                    default:
                        throw new Throwable\Parse\Exception('Unable to process Spring XML. Tag ":tag" has invalid child node ":child".', [':tag' => 'value', ':child' => $name]);
            ...
            if (is_string($value)) {
                ...
                if (isset($attributes['space'])) {
                    ...
                    if (!Spring\Data\XML\Syntax::isSpacePreserved($space)) {
                        throw new Throwable\Parse\Exception('Unable to process Spring XML. Expected a valid "space" token, but got ":token".', [':token' => $space]);
                    }
                } else {
                    $value = trim($value);
                }
