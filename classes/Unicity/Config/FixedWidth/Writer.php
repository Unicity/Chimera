...
class Writer extends Config\Writer
{
    ...
    protected function buildField(\SimpleXMLElement $node, $data, string $line)
    {
        ...
        if (isset($attributes['filler'])) {
            ...
            if (strlen($filler) != 1) {
                throw new Throwable\Parse\Exception('Unable to process template. Tag ":tag" defines an invalid filler character.', [':tag' => $node->getName()]);
            }
        } else {
            $filler = ' ';
        }
        ...
        if (isset($attributes['space'])) {
            ...
            if (!$this->isSpacePreserved($space)) {
                throw new Throwable\Parse\Exception('Unable to process template. Expected a valid space token, but got ":token".', [':token' => $space]);
            }
        } else {
            $value = trim($value);
        }
        ...
        foreach ($children as $child) {
            ...
            switch ($name) {
                case 'field':
                    ...
                    break;
                default:
                    throw new Throwable\Parse\Exception('Unable to process template. Tag ":tag" has invalid child node ":child".', [':tag' => $node->getName(), ':child' => $name]);
        ...
        foreach ($children as $child) {
            ...
            switch ($name) {
                ...
                case 'lines':
                    ...
                    break;
                default:
                    throw new Throwable\Parse\Exception('Unable to process template. Tag ":tag" has invalid child node ":child".', [':tag' => 'template', ':child' => $name]);
        ...
        try {
            ...
            switch ($name) {
                case 'template':
                    ...
                    break;
                default:
                    throw new Throwable\Parse\Exception('Unable to process template. Tag ":tag" cannot be found.', [':tag' => 'template']);
