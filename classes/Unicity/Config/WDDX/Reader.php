...
class Reader extends Config\Reader
{
        ...
        foreach ($children as $child) {
            ...
            switch ($name) {
                ...
                case '#text':
                    break;
                default:
                    throw new Throwable\Parse\Exception('Invalid child node named ":name" detected.', [':name' => $name]);
    ...
    protected function parseBooleanElement(\DOMElement $node)
    {
        ...
        if ($attribute === null) {
            throw new Throwable\Parse\Exception('Missing attribute named ":var".', [':var' => 'value']);
        }
        ...
        switch ($value) {
            ...
            case 'false':
                return false;
            default:
                throw new Throwable\Parse\Exception('Expected a boolean value, but got :value for the value.', [':value' => $value]);
        }
    }
    ...
    protected function parseDateTimeElement(\DOMElement $node)
    {
        ...
        if (!preg_match(Core\DateTime::ISO_8601_PATTERN, $value) && !preg_match(Core\DateTime::UNIVERSAL_SORTABLE_PATTERN, $value)) {
            throw new Throwable\Parse\Exception('Expected a date, but got this ":value" value.', [':value' => $value]);
        }
    ...
    protected function parseNumberElement(\DOMElement $node)
    {
        ...
        if (preg_match('/^[+-]?(0|[1-9][0-9]*)((\.[0-9]+)|([eE][+-]?(0|[1-9][0-9]*)))$/', $value)) {
            ...
            throw new Throwable\Parse\Exception('Expected an integer number, but got :value for the value.', [':value' => $value]);
        }
    ...
    protected function parseVarElement(\DOMElement $node)
    {
        ...
        if ($attribute === null) {
            throw new Throwable\Parse\Exception('Missing attribute named ":var".', [':var' => 'name']);
        }
        $key = $attribute->nodeValue;
        if (!$this->isKey($key)) {
            throw new Throwable\Parse\Exception('Invalid key passed to map.', [':var' => 'name']);
        }
