...
abstract class Writer extends Core\AbstractObject
{
    ...
    public static function factory($format, $data, $restrict = true)
    {
        ...
        if ($restrict && in_array(strtolower($name), ['inc', 'php'])) {
            throw new Throwable\Instantiation\Exception('Unable to initialize class. Unknown writer format, got ":format".', [':format' => $format]);
        }
        ...
        if (!class_exists($writer)) {
            ...
            foreach ($directories as $file) {
                if (!in_array($file, ['.', '..']) && (!is_dir($directory . DIRECTORY_SEPARATOR . $file) && (strtolower($file) == strtolower($name)))) {
