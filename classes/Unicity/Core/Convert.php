...
class Convert extends Core\AbstractObject
{
    ...
    public static function changeType($value, $type)
    {
        ...
        switch ($type2) {
            ...
            default:
                throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type1" to a(n) :type2.', [':type1' => gettype($value), ':type2' => $type]);
        }
    }
    ...
    public static function toArray($value): array
    {
        ...
        switch ($type) {
            ...
            default:
                throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type" to an array.', [':type' => $type]);
        }
    }
    ...
    public static function toBoolean($value): bool
    {
        ...
        switch ($type) {
            ...
            case 'object':
                ...
                } elseif ($value instanceof Core\Data\Undefined) {
                    return false;
                } else {
                    throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type" to a boolean.', [':type' => get_class($value)]);
                }
                // no break
            case 'string':
                return (in_array(strtolower($value), ['false', 'f', 'no', 'n', '0', 'null', 'nil', ''])) ? false : (bool) $value;
            default:
                throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type" to a boolean.', [':type' => $type]);
        }
    }
    ...
    public static function toChar($value): string
    {
        ...
        switch ($type) {
            case 'boolean':
                throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type" to a char.', [':type' => $type]);
            case 'double':
                throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type" to a char.', [':type' => $type]);
            case 'integer':
                return chr($value);
            ...
            case 'object':
                if (method_exists($value, 'toChar')) {
                    ...
                    throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type" to a char.', [':type' => get_class($value)]);
                }
                // no break
            case 'string':
    ...
    public static function toDateTime($value, $format = 'c'): string
    {
        ...
        switch ($type) {
            case 'boolean':
                throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type" to a datetime.', [':type' => $type]);
            case 'double':
                throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type" to a datetime.', [':type' => $type]);
            case 'integer':
            ...
            case 'object':
                if (method_exists($value, 'toDateTime')) {
                    ...
                    throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type" to a datetime.', [':type' => get_class($value)]);
                }
                // no break
            case 'string':
                $value = trim($value);
    ...
    public static function toDictionary($value): array
    {
        ...
        switch ($type) {
            ...
            default:
                throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type" to a dictionary.', [':type' => $type]);
        }
    }
        ...
        switch ($type) {
            ...
            case 'object':
                if (method_exists($value, 'toDouble')) {
                    ...
                    throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type" to a double.', [':type' => get_class($value)]);
                }
                // no break
            case 'string':
                $value = trim($value);
                ...
                if (preg_match('/^[+-]?([0-9]+)((\.[0-9]+)|([eE][+-]?(0|[1-9][0-9]*)))?$/', $value)) {
                    return doubleval($value);
                } elseif (preg_match('/^[+-]?([0-9]+)(,[0-9]{3})*(\.[0-9]+)?$/', $value)) {
                    return doubleval(preg_replace('/[^-+\d.]/', '', $value));
                } else {
                    throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type" to a double.', [':type' => $type]);
                }
                // no break
            default:
                throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type" to a double.', [':type' => $type]);
        ...
        switch ($type) {
            ...
            case 'object':
                if (method_exists($value, 'toInteger')) {
                    ...
                    throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type" to an integer.', [':type' => get_class($value)]);
                }
                // no break
            case 'string':
                $value = trim($value);
                ...
                if (!preg_match('/^[+-]?([0-9]+)((\.[0-9]+)|([eE][+-]?(0|[1-9][0-9]*)))?$/', $value)) {
                    throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type" to an integer.', [':type' => $type]);
                }
    ...
    public static function toList($value): Common\IList
    {
        ...
        switch ($type) {
            ...
            default:
                throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type" to a list.', [':type' => $type]);
        }
    }
    ...
    public static function toMap($value): Common\IMap
    {
        ...
        switch ($type) {
            ...
            default:
                throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type" to a map.', [':type' => $type]);
        }
    }
    ...
    public static function toObject($value)
    {
        ...
        switch ($type) {
            case 'array':
                ...
                throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type" to an object.', [':type' => $type]);
            case 'object':
                return $value;
            default:
                throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type" to an object.', [':type' => $type]);
    ...
    public static function toOrdinal($value): int
    {
        ...
        switch ($type) {
            case 'boolean':
                throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type" to an ordinal.', [':type' => $type]);
            case 'double':
                throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type" to an ordinal.', [':type' => $type]);
            case 'integer':
                return $value;
            ...
            case 'object':
                if (method_exists($value, 'toOrdinal')) {
                    ...
                    throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type" to an ordinal.', [':type' => get_class($value)]);
                }
                // no break
            case 'string':
                if (mb_strlen($value) != 1) {
    ...
    public static function toSet($value): Common\ISet
    {
        ...
        switch ($type) {
            ...
            default:
                throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type" to a set.', [':type' => $type]);
        }
    }
    ...
    public static function toString($value): string
    {
        ...
        switch ($type) {
            ...
            case 'string':
                return $value;
            default:
                throw new Throwable\Parse\Exception('Invalid cast. Could not convert value of type ":type" to a string.', [':type' => $type]);
        }
    }
