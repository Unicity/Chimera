...
class Exception extends \InvalidArgumentException implements Core\IObject
{
    ...
    public static function assertArrayKeyExists($collection, $key, $callee)
    {
        if (!array_key_exists($key, $collection)) {
            throw new static('Unable to execute :callee. Expected a valid key, but ":key", does not exist.', [
                ':callee' => $callee,
                ':key' => $key,
            ]);
        }
    ...
    public static function assertCallback($callback, $callee, $position)
    {
        if (!is_callable($callback)) {
            if (!is_array($callback) and !is_string($callback)) {
                throw new static('Unable to execute :callee. Expected a valid callback, but no array, closure, functor, or string was given at parameter position ":position".', [
                    ':callee' => $callee,
                    ':position' => $position,
                ]);
            }
            ...
            throw new static('Unable to execute :callee. Expected a valid callback, but got ":type" instead at parameter position ":position".', [
                ':callee' => $callee,
                ':position' => $position,
                ':type' => $type,
            ]);
    ...
    public static function assertCollection($collection, $callee, $position)
    {
        if (!is_array($collection) && !($collection instanceof \Traversable)) {
            throw new static('Unable to execute :callee. Expected an array or an instance of Traversable, but got a value of type":type" instead at parameter position ":position".', [
                ':callee' => $callee,
                ':position' => $position,
    ...
    public static function assertMethodName($method, $callee, $position)
    {
        if (!is_string($method) || !preg_match('/^[a-z_][a-z0-9_]*$/i', $method)) {
            throw new static('Unable to execute :callee(). Expected a "string", but got a value of type":type" instead at parameter position ":position".', [
                ':callee' => $callee,
                ':position' => $position,
                ':type' => gettype($method),
            ]);
    ...
    public static function assertPositiveInteger($value, $callee, $position)
    {
        if (((string) (int) $value !== (string) $value) || ($value < 0)) {
            ...
            throw new static('Unable to execute :callee(). Expected a "positive integer", but got a value of type ":type" instead at parameter position ":position".', [
                ':callee' => $callee,
                ':position' => $position,
                ':type' => $type,
            ]);
    ...
    public static function assertPropertyName($property, $callee, $position)
    {
        if (!is_string($property) && !is_integer($property) && !is_float($property) && !is_null($property)) {
            throw new static('Unable to execute :callee(). Expected a valid property name or array index, but got a value of type":type" instead at parameter position ":position".', [
                ':callee' => $callee,
                ':position' => $position,
    ...
    {
        $types = ['NULL', 'string', 'integer', 'double', 'boolean'];
        $type = gettype($key);
        if (!in_array($type, $types, true)) {
            throw new static('Unable to execute :callee(). Expected a valid array key, but got a value of type ":type".', [
                ':callee' => $callee,
                ':type' => $type,
            ]);
        }
