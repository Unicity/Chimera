...
class Factory extends MappingService\Data\Translator
{
    ...
    public function __call($method, $args)
    {
        ...
        } elseif (preg_match('/^set[_a-zA-Z0-9]+$/', $method)) {
            return $this->__setter('set', substr($method, 3), $args);
        } elseif (preg_match('/^uget[_a-zA-Z0-9]+$/', $method)) {
            return $this->__getter('uget', substr($method, 4));
        } elseif (preg_match('/^uset[_a-zA-Z0-9]+$/', $method)) {
        ...
        if (count($nodes) > 0) {
            ...
            foreach ($items as $item) {
                ...
                if (!isset($attributes['name'])) {
                    throw new Throwable\Parse\Exception('Unable to parse ":scope" method in translator.', [':scope' => $scope]);
                }
                $name = $this->__valueOf($attributes['name']);
    ...
    public function __hasMethod($method)
    {
        if (parent::__hasMethod($method)) {
            return true;
        } elseif (preg_match('/^(g|s)et[_a-zA-Z0-9]+$/', $method)) {
