        ...
        foreach ($source as $index => $source_value) {
            ...
            if ($this->doLog($new_path)) {
                if ($target->hasIndex($index)) {
                    ...
                    } elseif (($source_value instanceof Common\IMap) && ($target_value instanceof Common\IMap)) {
                    ...
                    $log->addValue([
                        'body' => strtr('Target index ":index" is missing in list.', [
                            ':index' => $index,
                        ]),
        ...
        foreach ($source as $key => $source_value) {
            ...
            if ($this->doLog($new_path)) {
                if ($target->hasKey($key)) {
                    ...
                    $log->addValue([
                        'body' => strtr('Target key ":key" is missing in map.', [
                            ':key' => $key,
                        ]),
                        'level' => Log\Level::warning()->__name(),
                        'path' => $new_path,
    ...
    protected function compareValues($source, $target, $path, Common\Mutable\IList $log)
    {
        ...
        if ($source_info->hash != $target_info->hash) {
            if ($source_info->type != $target_info->type) {
                $log->addValue([
                    'body' => strtr('Target value is of ":target" type, but source value is of ":source" type.', [
                ...
                $log->addValue([
                    ...
                    'body' => strtr('Target value ":target" is different from source value ":source".', [
    ...
    protected static function useCollections($data, $case_sensitive)
    {
        if (is_object($data)) {
            if (($data instanceof Common\IList) || ($data instanceof Common\ISet)) {
                ...
                return $buffer;
            } elseif (($data instanceof MappingService\Data\IModel) || ($data instanceof \stdClass)) {
