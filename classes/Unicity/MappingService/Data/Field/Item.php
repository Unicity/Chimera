...
class Item extends Core\AbstractObject
{
    ...
    public function __construct(string $name, $value)
    {
        ...
        if (($info->class == 'object') && ($info->type != 'string')) {
            throw new Throwable\InvalidArgument\Exception('Invalid argument specified. Expected a primitive or unknown, but got ":type"', [':type', $info->type]);
        }
        $this->info = $info;
        $this->name = $name;
        $this->value = $value;
    ...
    public function isRequired()
    {
        if ($this->info->type == 'string') {
            ...
            return $this;
        } elseif (Core\Data\ToolKit::isUnset($this->value)) {
            // TODO add logging
        }
    ...
    public function value(/*$default = \Unicity\Core\Data\Undefined::instance(), $eval = 'ifUndefined'*/)
    {
        ...
        if ($argc > 0) {
            ...
            return call_user_func_array(['\\Unicity\\MappingService\\Data\\ToolKit', $function], [$this->value, $default]);
        }
